"""Factory for the v5 known-size oracle grasp-pressure environment."""
...
def make_known_size_grasp_env(
    *,
    ...
    cfg.actions.arm_action.scale = float(
        camera_arm_scale if camera_name is not None else arm_scale
    )
    ...
    preserve_oracle_physics = camera_name is None or preserve_oracle_physics_with_camera
    ...
    if camera_name is not None:
        if not camera_name or min(int(camera_width), int(camera_height)) < 1:
            raise ValueError("camera_name must be non-empty and camera dimensions must be positive")
        from isaaclab.sensors import CameraCfg
        import isaaclab.sim as sim_utils

        data_types = tuple(str(value) for value in camera_data_types)
        if not data_types or any(value not in ("rgb", "distance_to_image_plane") for value in data_types):
            raise ValueError("camera_data_types must contain rgb and/or distance_to_image_plane")
        setattr(cfg.scene, camera_name, CameraCfg(
            ...
            offset=CameraCfg.OffsetCfg(
                pos=tuple(float(v) for v in camera_position),
                rot=tuple(float(v) for v in camera_rotation_wxyz),
                convention="ros",
            ),
        ))
...
def install_known_size_gripper_action(raw, known_size, effort_limit: float = 40.0):
    ...
    old_term = raw.unwrapped.action_manager.get_term("gripper_action")

"""Label-free physical conditioning for modular manipulation policies."""
...
from stage_vla.objects import ACTION_CONTEXT_VERSION


def snapshot_has_physical_context(payload: Mapping[str, object]) -> bool:
    """Return whether a snapshot carries the current physical-pose contract."""
    return (
...
class PhysicalDomainConfig:
    """Sampling bounds for one geometry/grasp-topology model bundle."""
    ...
    def sample(self, num_envs: int, *, seed: int) -> "PhysicalObjectBatch":
        ...
        return PhysicalObjectBatch(
            ...
            stackable=torch.full((int(num_envs),), self.stackable, dtype=torch.bool),
...
class PhysicalObjectBatch:
    """Per-environment physical values shared by simulation and policy code."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.context_version != ACTION_CONTEXT_VERSION:
            raise ValueError(f"unsupported physical context version: {self.context_version!r}")
    ...
    def from_action_context(
        ...
        *,
        ...
        """Decode the stable 10-D action context into physical profiles."""
    ...
    def from_snapshot_payloads(
        ...
        """Repeat snapshot profiles while preserving env-to-snapshot binding."""
        ...
        bundles = {str(payload.get("geometry_bundle", "")).strip() for payload in payloads}
        if len(bundles) != 1 or not next(iter(bundles)):
            raise ValueError("snapshot geometry_bundle values must match and be non-empty")
        rows = [payloads[index % len(payloads)]["physical_context"] for index in range(int(num_envs))]
    ...
    def action_context(self) -> Tensor:
        """Encode the stable 10-D, label-free action context ABI."""
        return torch.cat(
            [
...
class GeometryAdapter:
    """Convert physical dimensions into controller and stacking geometry.

    A box restored from a REACH snapshot may have any yaw relative to the
    parallel jaws.  Its exact projected grasp width therefore depends on live
    pose, not just one entry in ``size_xyz``.  The adapter exposes a conservative
    planar envelope: the shortest side defines the hard compression stop and
    the footprint diagonal defines a guaranteed-open pre-contact target.  The
    force loop then closes incrementally from its current target.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if min(float(self.jaw_clearance_m), float(self.max_compression_m), float(self.joint_min_m)) < 0:
            raise ValueError("jaw clearance, compression and joint minimum must be non-negative")
        if float(self.joint_min_m) >= float(self.joint_max_m):
            raise ValueError("joint_min_m must be less than joint_max_m")
        if torch.any(self.effective_width_m / 2 + float(self.jaw_clearance_m) > float(self.joint_max_m)):
    ...
    def geometric_joint_target_m(self) -> Tensor:
        return (self.effective_width_m / 2 + float(self.jaw_clearance_m)).clamp_max(
    ...
    def stack_center_separation_m(self) -> Tensor:
        return (self.physical.object_size_m[:, 2] + self.physical.support_size_m[:, 2]) / 2
    ...
    def pressure_feedback_step(
        ...
        *,
        ...
        target = torch.as_tensor(target_force_n, device=current.device, dtype=current.dtype)
        if current.shape != (self.physical.num_envs, 2) or measured.shape != current.shape:
        ...
        # Do not snap directly to a size-derived width.  For a yawed box that
        # width is only an envelope, and a snap can either crush a long side or
        # stop short of a narrow side.  Incremental force feedback discovers
        # contact while the geometry adapter remains the hard safety bound.
...
@dataclass(frozen=True)
class LoadAdapter:
    """Convert per-environment payload physics into bounded force targets."""
    ...
    def tracking_ok(self, measured_force_n: Tensor, target_force_n: Tensor) -> Tensor:
        ...
        error_ok = (measured - target.unsqueeze(-1)).abs().amax(dim=-1) <= float(
            self.pressure_tolerance_n
        )
        balance_ok = (measured[:, 0] - measured[:, 1]).abs() <= float(
            self.force_balance_tolerance_n
        )

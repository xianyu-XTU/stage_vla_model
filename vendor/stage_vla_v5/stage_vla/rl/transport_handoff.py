"""TRANSPORT terminal contract and near-goal settling reward."""
...
class TransportHandoffConfig:
    """Thresholds and shaping weights for a stationary ALIGN handoff."""
    ...
    def validate(self) -> None:
        ...
        if any(not torch.isfinite(torch.tensor(value)) or value <= 0 for value in positive):
            raise ValueError("TRANSPORT handoff thresholds must be finite and positive")
        if any(not torch.isfinite(torch.tensor(value)) or value < 0 for value in non_negative):
            raise ValueError("TRANSPORT handoff reward weights must be finite and non-negative")
...
def transport_handoff_ready(
    ...
    *,
    ...
    """Return the instantaneous stationary handoff predicate."""
    cfg.validate()
    ...
    linear = torch.as_tensor(linear_speed_mps, device=physical.device, dtype=torch.float32)
    angular = torch.as_tensor(angular_speed_radps, device=physical.device, dtype=torch.float32)
    if not (physical.shape == pressure.shape == xy.shape == linear.shape == angular.shape):
        raise ValueError("TRANSPORT handoff inputs must have matching shapes")
    if not torch.isfinite(xy).all() or not torch.isfinite(linear).all() or not torch.isfinite(angular).all():
...
def transport_settle_reward_terms(
    ...
    *,
    ...
    """Shape braking only near the handoff region without overriding actions."""
    cfg.validate()
    ...
    if not all(torch.isfinite(value).all() for value in (xy, linear, angular, action)):
    ...
    return {
        "approach": approach,
        "linear_speed_cost": approach * float(cfg.linear_speed_weight) * linear_ratio,
        "angular_speed_cost": approach * float(cfg.angular_speed_weight) * angular_ratio,
        "action_cost": (
            approach * float(cfg.action_weight) * action[:, :2].square().sum(dim=-1)
        ),
        "settled_bonus": (
            approach

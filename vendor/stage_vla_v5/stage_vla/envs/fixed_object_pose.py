"""Reset-time helpers for deterministic cube positions.

The Isaac Lab stack task randomizes all three cubes on reset.  v5's known-size
chain only consumes ``cube_1`` (blue) and ``cube_2`` (red), so this event term
overrides those two positions after the standard randomization event.  The
coordinates are expressed in the local environment frame, matching the
``pose_range`` used by the upstream task configuration.
"""
...
def _xyz(value: Sequence[float], *, name: str, device: torch.device) -> torch.Tensor:
    """Convert and validate one local XYZ coordinate or an ``[N, 3]`` batch."""

    result = torch.as_tensor(value, dtype=torch.float32, device=device)
    if result.ndim not in (1, 2) or result.shape[-1] != 3:
        raise ValueError(f"{name} must contain [x, y, z] or a batch with shape [N, 3]")
...
def _selected_xyz_rows(
    ...
    *,
    ...
    """Broadcast one XYZ row or select rows from an environment-sized batch."""
    if value.ndim == 1:
...
def set_fixed_pair_pose(
    ...
    *,
    ...
    """Set the blue and red cube positions for the selected environments.

    ``env_ids`` is supplied by Isaac Lab's reset event manager.  Existing
    cube orientations are preserved (the user is specifying position only),
    and linear/angular velocities are cleared so the reset starts from rest.
    The minimum separation check prevents the two 4-cm cubes from being
    spawned interpenetrating one another; the upstream task's usual sampling
    range is wider (0.1 m).
    """
    ...
    if blue.ndim == 2 and blue.shape[0] not in (len(ids), env.num_envs):
        raise ValueError("batched fixed positions must have one row per selected or total environment")
    xy_distance = torch.linalg.vector_norm(blue[..., :2] - red[..., :2], dim=-1)
    ...
    if bool(too_close.any()):
        ...
        raise ValueError("blue_xyz and red_xyz are too close in XY at "
    ...
    for name, local_xyz in ((blue_name, blue), (red_name, red)):
        ...
        # Preserve the orientation generated by the normal reset event.  This
        # avoids relying on a quaternion ordering when only XYZ was requested.
...
def set_fixed_asset_poses(
    ...
    *,
    ...
    """Set rigid-object positions from broadcast XYZ or per-environment rows."""
    if env_ids is None or len(env_ids) == 0:
        return
    if len(asset_xyz) < 2:

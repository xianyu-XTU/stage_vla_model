"""Isaac adapters for deterministic per-environment physical profiles."""
...
def verify_rigid_body_mass_profile(asset, expected_kg: torch.Tensor) -> torch.Tensor:
    ...
    if actual.shape != expected.shape or not torch.allclose(actual, expected, atol=1e-6, rtol=1e-5):

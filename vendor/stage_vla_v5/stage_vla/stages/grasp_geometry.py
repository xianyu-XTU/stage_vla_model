"""Geometry-conditioned grasp targets for rigid object families.

This module is deliberately independent of Isaac Lab and policy code.  It
turns a bounding-box description into a local parallel-jaw target so callers
can share the same geometry rule during data capture, training and evaluation.
"""
...
class GraspGeometryProfile:
    """Object-family parameters used to choose a side grasp."""
...
class ParallelJawCandidate:
    """A geometry-only grasp target consumed by the REACH controller."""
...
def parallel_jaw_yaw_error(
    ...
    """Return the shortest yaw error to either horizontal box principal axis.

    Parallel jaws are symmetric after a half turn, so the result is wrapped to
    ``[-pi/4, pi/4]`` by choosing the nearer of the local X and Y axes.  Both
    planar dimensions in a compatible geometry bundle must fit the gripper;
    avoiding an unnecessary 90-degree wrist turn preserves IK reachability.
    The calculation is label-free and uses only live pose, metric dimensions
...
def profile_for_geometry(geometry: str, *, height_ratio: float | None = None) -> GraspGeometryProfile:
    """Return conservative defaults for a rigid geometry family."""
    if geometry == "cone":
        profile = GraspGeometryProfile(geometry, 0.25 if height_ratio is None else height_ratio)
    else:
        profile = GraspGeometryProfile(geometry, 0.5 if height_ratio is None else height_ratio)
    profile.validate()
    return profile


def profile_from_config(path: str | Path, *, geometry: str | None = None) -> GraspGeometryProfile:
    """Load the optional geometry profile from a task JSON configuration."""
    ...
    if "width_ratio" in grasp:
        profile = GraspGeometryProfile(profile.geometry, profile.height_ratio, float(grasp["width_ratio"]))
        profile.validate()
    return profile
...
def local_width_ratio(
    ...
    *,
    ...
    """Compute the local cross-section / bounding-width ratio.

    ``height_ratio`` is measured from the object's bottom.  A base-down cone
    therefore has local width ``1 - height_ratio``.  Box and cylinder remain
    constant-width families.
    """
    profile = GraspGeometryProfile(geometry, float(height_ratio) if not isinstance(height_ratio, Tensor) else 0.5, explicit_width_ratio)
    profile.validate()
...
def build_parallel_jaw_candidate(
    ...
    *,
    ...
    """Build a batched side-grasp target from an object's world pose."""
    profile.validate()
    ...
    if size.shape not in ((3,), position.shape) or not torch.isfinite(size).all() or (size <= 0).any():
        raise ValueError("size_xyz_m must be [3] or match object_position_w")
    ratio = torch.as_tensor(profile.height_ratio, dtype=position.dtype, device=position.device)
    ...
    return ParallelJawCandidate(target, effective_width, float(profile.height_ratio), width_ratio)
...
def grasp_target_position(
    ...
    *,
    ...
    """Convenience wrapper returning only the target position."""
    return build_parallel_jaw_candidate(object_position_w, size_xyz_m, profile=profile).target_position_w

...
def main() -> None:
    ...
    p.add_argument("--video_fps", type=float, default=20.0)
    p.add_argument(
    ...
    if args.use_vision and args.video:
        p.error("--video is not supported together with --use_vision")
    ...
    if args.video and min(args.video_width, args.video_height) < 1:
        p.error("video dimensions must be positive")
    if args.video and args.video_fps <= 0:
    ...
    if asset_layout_file is not None:
        ...
        from stage_vla.evaluation.four_cube_stack import load_layout_manifest
    ...
    if args.random_xy:
        rng = random.Random(args.seed if args.random_xy_seed is None else args.random_xy_seed)
        blue_positions, red_positions = [], []
        for _ in range(args.num_envs):
            for _attempt in range(10000):
                blue = [rng.uniform(0.4, 0.6), rng.uniform(-0.1, 0.1), 0.0203]
                red = [rng.uniform(0.4, 0.6), rng.uniform(-0.1, 0.1), 0.0203]
                if math.dist(blue[:2], red[:2]) >= 0.1:
                    blue_positions.append(blue)
                    red_positions.append(red)
                    break
            else:
                raise RuntimeError("could not sample 20 valid red/blue XY pairs")
        object_positions = {"blue": blue_positions, "red": red_positions}
    ...
    try:
        ...
        raw = make_known_size_grasp_env(
            ...
            camera_width=128 if args.use_vision else args.video_width,
            camera_height=128 if args.use_vision else args.video_height,
            camera_data_types=("rgb", "distance_to_image_plane") if args.use_vision else ("rgb",),
        ...
        video_camera = (
            raw.unwrapped.scene["v5_chain_video_camera"] if args.video else None
        )
        video_frames: list[tuple[object, str, int]] = []
        ...
        def capture_video_frame(stage: str, step: int) -> None:
            ...
            rgb = torch.as_tensor(video_camera.data.output["rgb"])[args.video_env, ..., :3]
            ...
            rgb = rgb.to(torch.uint8).detach().cpu().numpy()
            video_frames.append((rgb, stage, int(step)))
        ...
        if args.use_vision:
            vision_camera = raw.unwrapped.scene["v7_multicube_camera"]
        ...
        def visual_asset_positions():
            ...
            rgb = read_rgb_u8_batch(raw.unwrapped, camera_name="v7_multicube_camera")
            depth = read_depth_m_batch(raw.unwrapped, camera_name="v7_multicube_camera")
            predicted = {
        ...
        if args.use_vision:
            initial_rgb = read_rgb_u8_batch(
                raw.unwrapped, camera_name="v7_multicube_camera"
            )[0]
        ...
        if args.video and video_frames:
            ...
            try:
                for rgb, stage, step in video_frames:
                    image = Image.fromarray(rgb, "RGB")
                    canvas_h = image.height + 40
                    canvas = Image.new("RGB", (image.width, canvas_h), (15, 15, 20))
                    canvas.paste(image, (0, 40))
                    draw = ImageDraw.Draw(canvas)
                    draw.text(
                        (8, 6), f"seed {args.seed}  env {args.video_env}  step {step}",
                        font=font_small, fill=(230, 230, 230),
                    )
                    draw.text((8, image.height + 6), stage, font=font_big,
                              fill=(120, 220, 255))

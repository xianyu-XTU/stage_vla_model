"""Physical-domain contracts for complete action-policy bundles."""
    ...
    def __post_init__(self) -> None:
        for lower, upper in (
            ...
            if (
                len(lower) != 3
                or len(upper) != 3
                or any(a <= 0 or a > b for a, b in zip(lower, upper))
            ):
...
@dataclass(frozen=True)
class ActionBundle:
    """One complete set of registered skill policies sharing a physical domain."""

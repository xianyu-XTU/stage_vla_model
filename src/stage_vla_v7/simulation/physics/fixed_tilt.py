"""Pure-Torch fixed roll/pitch reference for the optional Isaac IK adapter."""
...
def euler_xyz_from_quat(
    ...
    roll = torch.atan2(
        2 * (qw * qx + qy * qz), 1 - 2 * (qx.square() + qy.square())
    )
    sin_pitch = (2 * (qw * qy - qz * qx)).clamp(-1, 1)
    pitch = torch.asin(sin_pitch)
    yaw = torch.atan2(
        2 * (qw * qz + qx * qy), 1 - 2 * (qy.square() + qz.square())
    )
...
class FixedTiltReference:
    ...
    def target(
        self, current_quat: torch.Tensor, delta_yaw: torch.Tensor
    ) -> torch.Tensor:
        if (
            current_quat.shape != (len(self.initialized), 4)
            or delta_yaw.shape != self.desired_yaw.shape
        ):
        ...
        self.reference_rpy[pending] = torch.stack(
            (roll[pending], pitch[pending], yaw[pending]), dim=-1
        )
        ...
        return quat_from_euler_xyz(
            self.reference_rpy[:, 0],
            self.reference_rpy[:, 1],
            self.desired_yaw,
        )


def summarize_tilt_trace(
    trace: dict, *, max_tilt_error_rad: float = 0.035
) -> dict:
    """Summarize actual and desired tilt errors without altering task scoring."""
    frames = trace.get("frames", {})
    required = (
        "ee_quat_xyzw",
        "ik_desired_quat_xyzw",
        "tilt_reference_rpy",
        "tilt_initialized",
    )
    ...
    actual_error = torch.maximum(
        wrap_angle(ar - reference[..., 0]).abs(),
        wrap_angle(ap - reference[..., 1]).abs(),
    )
    desired_error = torch.maximum(
        wrap_angle(dr - reference[..., 0]).abs(),
    ...
    for env in range(actual.shape[1]):
        ...
        rows.append(
            {
                "env": env,
                "max_actual_tilt_error_rad": actual_max,
    ...
    return {
        "version": "M31-fixed-tilt-diagnostic-v1",
        "max_tilt_error_rad": max_tilt_error_rad,
        "rows": rows,

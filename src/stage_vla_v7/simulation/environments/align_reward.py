"""Training-only dense reward terms for the ALIGN Skill."""
...
def align_reward_terms(
    ...
    *,
    ...
    vectors = (
        ...
        torch.as_tensor(
            before_height_error_m, device=after_xy.device, dtype=after_xy.dtype
        ),
        torch.as_tensor(
            after_height_error_m, device=after_xy.device, dtype=after_xy.dtype
        ),
        torch.as_tensor(
            stability_speed_mps, device=after_xy.device, dtype=after_xy.dtype
        ),
        torch.as_tensor(
            after_angular_speed_radps,
            device=after_xy.device,
            dtype=after_xy.dtype,
        ),
        torch.as_tensor(
            before_upright_tilt_rad,
            device=after_xy.device,
            dtype=after_xy.dtype,
        ),
        torch.as_tensor(
            after_upright_tilt_rad, device=after_xy.device, dtype=after_xy.dtype
        ),
        torch.as_tensor(
            left_grasp_height_error_m,
            device=after_xy.device,
            dtype=after_xy.dtype,
        ),
        torch.as_tensor(
            right_grasp_height_error_m,
            device=after_xy.device,
            dtype=after_xy.dtype,
        ),
        torch.as_tensor(
            grasp_height_tolerance_m,
            device=after_xy.device,
            dtype=after_xy.dtype,
        ),
    )
    ...
    if (
        unit.shape != (len(after_xy), 5)
        or requested.shape != unit.shape
        or previous.shape != unit.shape
    ):
        raise ValueError("ALIGN reward actions must all have shape [N,5]")
    if not all(
        torch.isfinite(value).all()
        for value in (after_xy, *vectors, unit, previous, requested)
    ):

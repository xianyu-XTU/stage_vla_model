"""V7-owned Isaac environment for the frozen known-size cube policies.

The concrete class keeps the historical simulator behavior while delegating
physical-state, observation, control, and terminal contracts to V7-owned
components. V5 code is used only by separate regression tests.
"""
...
from stage_vla_v7.simulation.environments.known_size_observation import (
...
from .action_adapter import known_size_raw_action
from .gripper_action import KnownSizeGraspAction
...
LEGACY_OBS_DIM = LEGACY_OBSERVATION_DIM
...
class KnownSizeEnvironmentStatus:
    """Read-only view of policy and terminal state for evaluation clients."""
    ...
    lift_target_height_m: torch.Tensor


@dataclass(frozen=True)
class KnownSizeGripperDiagnostics:
    """Read-only gripper controller outputs used in physical audit rows."""

    target_force_n: torch.Tensor
    joint_target_m: torch.Tensor
...
class KnownSizeGraspEnvironment(RslRlVecEnvWrapper):
    """Vectorized V7 physical environment for the frozen five-value policy ABI."""
    ...
    def __init__(
        ...
        *,
        ...
        self._stability_speed_source = stability_speed_source
    ...
    def _measure(self):
        ...
        return PhysicalState(
            object_position=placement.object_position_w.clone(),
            support_position=placement.support_position_w.clone(),
            object_linear_velocity=placement.object_linear_velocity_w.clone(),
            object_angular_velocity=placement.object_angular_velocity_w.clone(),
            object_orientation=red_quat,
            support_orientation=blue_quat,
            gripper_open=placement.gripper_open.clone(),
            gripper_joint_position=placement.gripper_joint_position.clone(),
            object_speed=placement.object_linear_velocity_w.norm(dim=-1),
            end_effector_position=grasp.end_effector_position_w.clone(),
            left_fingertip_position=grasp.left_fingertip_position_w.clone(),
            right_fingertip_position=grasp.right_fingertip_position_w.clone(),
            fingertip_force=force,
            physical_grasp=physical_diag.physical_grasp,
            ...
            radial_error=physical_diag.radial_error_m,
            left_height_error=physical_diag.left_height_error_m,
            right_height_error=physical_diag.right_height_error_m,
            projection_alpha=physical_diag.projection_alpha,
            fingertip_gap=physical_diag.fingertip_gap_m,
            arm_joint_position=to_torch(self.robot.data.joint_pos)[:, self.arm_ids].clone(),
            arm_joint_velocity=to_torch(self.robot.data.joint_vel)[:, self.arm_ids].clone(),
        )

    def get_physical_state(self) -> PhysicalState:
        """Return a fresh typed snapshot of simulator physical feedback."""
        return self._measure()

    @property
    def physical_state(self) -> PhysicalState:
        """Return the latest physical state synchronized by reset or step."""
        if not isinstance(self.measured, PhysicalState):
            raise RuntimeError("physical measurements are not initialized")
        return self.measured

    @property
    def status(self) -> KnownSizeEnvironmentStatus:
        """Expose terminal and policy bookkeeping without writable attributes."""
        return KnownSizeEnvironmentStatus(
            finished=self.finished,
            success=self.last_success,
            failure=self.last_failure,
            timeout=self.last_timeout,
            stable_count=self.stable_count,
            previous_action=self.prev_unit,
            lift_target_height_m=self.lift_target_height_m,
        )

    @property
    def gripper_diagnostics(self) -> KnownSizeGripperDiagnostics:
        """Expose the gripper command state needed by evaluator diagnostics."""
        return KnownSizeGripperDiagnostics(
            target_force_n=self.gripper_action.target_force_n,
            joint_target_m=self.gripper_action.processed_actions,
        )

    @property
    def object_size_m(self) -> torch.Tensor:
        """Return the per-environment object dimensions used by the controller."""
        return self.physical_batch.object_size_m

    @property
    def current_skill(self) -> str:
        """Return the active downstream Skill name."""
        return self.skill

    @property
    def stability_speed_source(self) -> str:
        """Return the configured source for stability terminal checks."""
        return self._stability_speed_source

    def configure_evaluation(
        self,
        *,
        auto_reset: bool,
        physical_height_tolerance_m: float | None = None,
    ) -> None:
        """Configure evaluator-only reset and physical diagnostic behavior."""
        self.auto_reset = bool(auto_reset)
        if physical_height_tolerance_m is None:
            return
        tolerance = float(physical_height_tolerance_m)
        if not np.isfinite(tolerance) or tolerance <= 0.0:
            raise ValueError("physical_height_tolerance_m must be positive and finite")
        self.physical_cfg = replace(
            self.physical_cfg,
            height_tolerance_m=tolerance,
        )

    def observe(self) -> TensorDict:
        """Return the current frozen policy observation through the public API."""
        if self.measured is None:
            return self.reset()[0]
        return self._obs()

    def synchronize_after_external_step(self) -> PhysicalState:
        """Refresh wrapper state after native Isaac stepping outside ``step``."""
        measured = self.get_physical_state()
        self.measured = measured
        self.entry_red_z[:] = measured["red"][:, 2]
        self.prev_force[:] = measured["force"]
        return measured
    ...
    def configure_skill(
        ...
        *,
        ...
        """Switch Skill bookkeeping without changing simulator physical state."""
        if skill not in (
            "GRASP",
            "LIFT",
            "TRANSPORT",
            ...
            raise ValueError("unsupported known-size skill")
        if int(episode_steps) < 1 or int(stable_steps) < 1:
            raise ValueError("episode_steps and stable_steps must be positive")
        active = torch.as_tensor(active_mask, dtype=torch.bool, device=self.device)
        if active.shape != (self.num_envs,):
            raise ValueError(f"active_mask must have shape ({self.num_envs},)")

        before = self.get_physical_state()
        self.skill = skill
        self.mark_continuous_handoff()
        self.arm_locked = False
        self.episode_steps = int(episode_steps)
        self.max_episode_length = int(episode_steps)
        self.stable_steps = int(stable_steps)
        self.steps.zero_()
    ...
    def _obs(self):
        if not isinstance(self.measured, PhysicalState):
            raise RuntimeError("physical measurements are not initialized")
        obs = build_known_size_observation(KnownSizeObservationInput(
            state=self.measured,
            target_force_n=self.gripper_action.target_force_n,
            max_force_n=self.known_size.max_force_n,
            size_features=self.geometry_adapter.legacy_size_features(),
            previous_action=self.prev_unit,
            ...
            visual_object_position=self.visual_red,
            visual_support_position=self.visual_blue,
            physical_context=(
                self.physical_batch.action_context()
    ...
    def step(self, actions):
        ...
        stability_speed = stability_speed_from_source(
            after["speed"], control_speed, self.stability_speed_source
        )
        control_angular_speed = quaternion_control_angular_speed(
            before["red_quat"],
            after["red_quat"],
            step_dt_s=max(float(self.unwrapped.step_dt), 1e-6),
        )
        instantaneous_angular_speed = after["angular"].norm(dim=-1)
        stability_angular_speed = stability_speed_from_source(
            instantaneous_angular_speed,
            control_angular_speed,
            self.stability_speed_source,
        )
        after = after.with_motion_metrics(
            control_speed=control_speed,
            stability_speed=stability_speed,
            control_angular_speed=control_angular_speed,
            instantaneous_angular_speed=instantaneous_angular_speed,
            stability_angular_speed=stability_angular_speed,
        )
        pressure_ok = self.gripper_action.pressure_tracking_ok(after["force"])
        terminal = self.success_checker.evaluate_physical_runtime(
            self.skill,
            after,
            ...
            stable_count=self.stable_count,
            stable_steps=self.stable_steps,
            prepress_steps=self.prepress_steps,
            entrance_warmup_steps=self.entrance_arm_warmup_steps,
            entry_object_z=self.entry_red_z,
            lift_target_height_m=self.lift_target_height_m,
            align_target_height_m=self.align_target_height_m,
            descend_target_height_m=self.descend_target_height_m,
            thresholds=PhysicalSkillThresholds(
                ...
                release_xy_m=self.release_xy_m,
                ...
                retreat_speed_mps=self.retreat_speed_mps,
            ),
        )
        self.stable_count = terminal.stable_count

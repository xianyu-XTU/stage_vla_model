...
- Evaluator compatibility entry reduced from 1,929 lines to 17 lines;
  whole-episode execution and result writing live under `tools/evaluation`.
...
- Dual-camera headless physical smoke passes 1/1 with a decodable 640 x 480,
  20 FPS MP4 containing 768 frames; Observer data was not used for Vision.
- Dual-camera audit: 730 V7 Vision calls, zero invalid frames, all eight prepared
...
| Evaluator split | PASS | 17-line entry plus evaluation/Simulation owners |
| VideoRecorder | PASS | unit tests and decoded physical MP4 |
| Dual Camera | PASS | isolated bindings and physical two-camera run |
| Vision + Video | PASS | 730 Vision calls and 768 video frames together |
...
- One physical seed verifies preserved wiring and behavior, not a multi-seed
  generalization rate.
- The whole physical episode loop remains intentionally centralized in
  `tools/evaluation/episode_runner.py`; backend construction, randomization,
  recording, overlays, and result persistence are no longer implemented there.
